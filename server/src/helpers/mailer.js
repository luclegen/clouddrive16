...
module.exports.sendCode = async (email, title, code) => {
  const options = {
    from: `"${process.env.NAME}" <${process.env.SERVER_EMAIL}>`,
    to: email,
    subject: title,
    text: '',
    html: `<h1><img src="${process.env.EMAIL_LOGO}" alt="${process.env.NAME} Logo"> ${process.env.NAME}</h1><hr><h1>` + title + '</h1><p>Verification code: ' + code + '</p>'
  }
