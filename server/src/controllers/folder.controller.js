          ...
          .then(folders =>
            folders.length
              ...
              : Folder.findByIdAndUpdate(req.params.id, { $set: { name: req.body.name } }, { new: true })
                .then(folderEdited =>
                  folderEdited
                    ? fs.rename(process.env.UPLOADS + req._id + '/files' + (folder.path === '/' ? folder.path : folder.path + '/') + folder.name, process.env.UPLOADS + req._id + '/files' + (folderEdited.path === '/' ? folderEdited.path : folderEdited.path + '/') + folderEdited.name, err => err) && res.status(200).send({ msg: 'Folder updated.' })
      ...
      folder
        ? folder.is_trash
          ? Folder.findByIdAndDelete(req.params.id)
            .then(folder => {
              folder
                ? fs.rm(process.env.UPLOADS + req._id + '/files' + (folder.path === '/' ? folder.path : folder.path + '/') + folder.name, { recursive: true }, err => err) && res.status(200).send({ msg: 'Folder permanently deleted.' })
                : res.status(404).send({ msg: 'Folder not found.' })
            })
            .catch(err => next(err))
