...
import { verify } from "../../pages/Home/slice";
...
export default function Activate() {
  ...
  const getIndex = (target) =>
    Array.from(document.querySelector(".row-code").children).findIndex(
      (i) => i === target
    );
  ...
  const enterNumber = (e) => {
    const [i, input] = [
      getIndex(e.target.closest(".col-md-digit")),
      getInput(),
    ];
  ...
  const getCode = () =>
    Array.from(document.querySelectorAll(".form-control-digit"))
      .map((e) => e.value)
      .join("");
  ...
  const isSubmit = () =>
    "0"
      ...
      .map(
        (v, i) =>
          (v = document.querySelectorAll(".form-control-digit")[i].value)
      )
      ...
      <form className="form-only" onSubmit={submit}>
        ...
        <div className="row-code">
          {"0"
            ...
            .split("")
            .map((v, i) => (
              <div className="col-md-digit" key={i}>
