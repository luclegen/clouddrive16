  ...
  rename = () =>
    helper.getType() === 'folder'
      ? folderService.read(helper.getId())
        .then(res => folderService.update(helper.getId(), { name: prompt('Rename folder', res.data.folder.name) }) && window.location.reload())
      : fileService.read(helper.getId())
        .then(res => fileService.update(helper.getId(), { name: prompt('Rename file', res.data.file.name) }) && window.location.reload())
