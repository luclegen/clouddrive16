    ...
    <main className="main-content col-10">
      ...
      {!this.isEmpty() && <ul className="ls-folder">
        {this.state.items.map((v, i, a) => a.length ? <li className="li-folder" key={i} id={v._id} name={v.name} title={v.name} onClick={this.open} onContextMenu={this.choose}>
          ...
          {helper.isImages(this.state.files, v) ? <img className="img" src={`${process.env.NODE_ENV === 'production' ? window.location.origin + '/api' : process.env.REACT_APP_API}/images/?image=${helper.getCookie('id')}/files${helper.getImage(this.state.files, v).path === '/' ? '/' : helper.getImage(this.state.files, v).path + '/'}${helper.getImage(this.state.files, v).name}`} alt="foreground folder" /> : helper.isVideos(this.state.files, v) ? <video className="video-preview" src={`${process.env.NODE_ENV === 'production' ? window.location.origin + '/api' : process.env.REACT_APP_API}/images/?image=${helper.getCookie('id')}/files${helper.getVideo(this.state.files, v).path === '/' ? '/' : helper.getVideo(this.state.files, v).path + '/'}${helper.getVideo(this.state.files, v).name}`} ></video > : !helper.isEmpty(this.state.folders, this.state.files, v) && <div className="file"></div>}
        ...
        {this.state.itemFiles.map((v, i) => <li className="li-file" key={i} id={v._id} name={v.name} title={v.name} onClick={this.open} onContextMenu={this.choose}>
          {helper.isImage(v.name) ? <img className="bg-img" id={`file${i}`} src={`${process.env.NODE_ENV === 'production' ? window.location.origin + '/api' : process.env.REACT_APP_API}/images/?image=${helper.getCookie('id')}/files${v.path === '/' ? '/' : v.path + '/'}${v.name}`} alt={`Img ${i}`} /> : helper.isVideo(v.name) ? <video width="118" height="86" src={`${process.env.NODE_ENV === 'production' ? window.location.origin + '/api' : process.env.REACT_APP_API}/images/?image=${helper.getCookie('id')}/files${v.path === '/' ? '/' : v.path + '/'}${v.name}`}></video> : <i className="material-icons bg-file">{helper.isAudio(v.name) ? 'audio_file' : 'description'}</i>}
